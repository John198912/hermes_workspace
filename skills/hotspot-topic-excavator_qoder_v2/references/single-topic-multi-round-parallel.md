...
```
第一轮：覆盖广度 + 深度（4路并行）
├─ WebSearch #1: 核心叙事（如 "Fable 5 Mythos controversy June 2026"）
├─ WebSearch #1: 时效信号（如 "Fable 5 export controls Trump"）
├─ WebSearch #2: 辅助角度（如 "Fable 5 NSA red team John Jumper Anthropic"）
└─ WebSearch #2: 争议/恢复（如 "Fable 5 unban restored Amazon SK Telecom"）

第二轮：补漏 + 最新预测（4路并行）
├─ WebSearch: freshness=pd, 最新恢复预测
├─ WebSearch: freshness=pd, 最新预测市场数据
├─ WebSearch: Amazon whistleblower + Trump statement
└─ WebSearch: open-weight model response
