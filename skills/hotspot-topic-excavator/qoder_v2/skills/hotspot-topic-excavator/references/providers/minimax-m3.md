...
2. **API Key 写入 `.env` 在 Feishu DM session**：terminal 写入含特殊字符的 key 到 `.env` 时，审批超时机制频繁拦截。需用户在自己终端手动执行 `echo 'MINIMAX_API_KEY=...' >> .env`。
3. **config.yaml 写入保护**：`patch`/`write_file` 直接写 `config.yaml` 会被拒绝。必须用 `terminal` + Python 脚本编辑，或 `qoder config set`。
